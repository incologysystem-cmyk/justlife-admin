...
export async function GET(req: NextRequest) {
  try {
    ...
    const upstream = new URL(`${base()}/api/notifications${qs}`);
    ...
    console.error("GET /api/provider/notifications error:", e?.message);
    console.dir(e?.body, { depth: 20 });

    return NextResponse.json(
      { ok: false, message: e?.message || "Failed", details: e?.body ?? null },
      { status: e?.status ?? 500 }
    );
  }
}
